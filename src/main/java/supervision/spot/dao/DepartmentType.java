...
public final class DepartmentType {
    // 数据库表名
    public static final String DATABASE_TABLE_NAME = "svp_department-type";

    private Connection connection;

    public DepartmentType(final Connection connection) throws Exception {
     ...
     * @return 消息对象
     */
    public final Message addDepartmentType(final String name, final Integer order) {
            ...
            // 添加单位类型
            {
                ...
                try {
                    ...
                    if (0 >= res) {
                        ...
                        msg.setContent("ADD_DEPARTMENT_TYPE_FAIL");
                        msg.setAttach("添加单位类型失败");
                        return msg;
                    }
                } finally {
                    if (null != ps) {
     ...
     * @param uuid 单位类型的uuid
     * @return 消息对象
     */
    public final Message removeDepartmentTypeByUuid(final boolean checkExist, final String uuid) {
        final Message msg = new Message();
        try {
            // 是否存在单位类型
            {
                if (checkExist) {
                    if (!this.isExistDepartmentType(uuid, null, null)) {
                        msg.setStatus(Message.Status.ERROR);
            ...
            // svp_department
            {
                ...
                if (obj.isExistDepartment(null, null, uuid, null, null)) {
                    msg.setStatus(Message.Status.ERROR);
                    msg.setContent("ASSOCIATED_DATA_DEPARTMENT_IS_EXIST");
                    msg.setAttach("存在关联数据单位");
                    return msg;
                }
            }
            // 删除单位类型
            {
                ...
                try {
                    ...
                    if (0 >= res) {
                        msg.setStatus(Message.Status.ERROR);
                        msg.setContent("REMOVE_DEPARTMENT_TYPE_FAIL");
                        msg.setAttach("删除单位类型失败");
                        return msg;
                    }
                } finally {
                    if (null != ps) {
     ...
     * 
     * @param uuid 角色的uuid
     * @param name 单位类型名称（允许为null）
     * @param order 排序编号（允许为null）
     * @return 消息对象
     */
    public final Message modifyDepartmentType(final String uuid, final String name, final Integer order) {
        final Message msg = new Message();
        try {
            // 是否存在单位类型
            {
            ...
            // 修改单位类型
            {
                ...
                try {
                    ...
                    if (0 >= res) {
                        msg.setStatus(Message.Status.ERROR);
                        msg.setContent("MODIFY_DEPARTMENT_TYPE_FAIL");
                        msg.setAttach("修改单位类型失败");
                        return msg;
                    }
                } finally {
                    if (null != ps) {
     ...
     * 获取单位类型
     * 
     * @param uuid 单位类型的uuid（允许为null）
     * @param name 单位类型名称（允许为null）
     * @return 消息对象
     */
    public final Message getDepartmentType(final String uuid, final String name) {
        final Message msg = new Message();
        try {
     ...
     */
    public final boolean isExistDepartmentType(final String uuid, final String name, final String excludeUuid) throws Exception {
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            final JSONArray whereArray = new JSONArray();
